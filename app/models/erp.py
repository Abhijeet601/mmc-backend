...
class ERPStudent(Base):
    ...
    reset_attempt_count: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
    reset_last_attempt_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
...
class ERPApplicationPayment(Base):
    ...
    payment_mode: Mapped[str] = mapped_column(String(50), default="CCAvenue", nullable=False)
    status: Mapped[str] = mapped_column(String(20), default="pending", nullable=False)
    order_id: Mapped[str | None] = mapped_column(String(100), unique=True, nullable=True, index=True)
    tracking_id: Mapped[str | None] = mapped_column(String(100), nullable=True, index=True)
    bank_ref_no: Mapped[str | None] = mapped_column(String(100), nullable=True)
    payment_status: Mapped[str | None] = mapped_column(String(30), nullable=True, index=True)
    ...
    encrypted_request: Mapped[str | None] = mapped_column(Text, nullable=True)
    encrypted_response: Mapped[str | None] = mapped_column(Text, nullable=True)
    decrypted_response: Mapped[str | None] = mapped_column(Text, nullable=True)
